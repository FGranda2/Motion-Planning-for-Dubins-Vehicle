...
def rrt_star_planner(rrt_dubins, display_map=False):
    ...
    random.seed(a=155)
    gamma = 20
    # Create dictionary for children list
    children = {}
    while i < rrt_dubins.max_iter:
        ...
        # For loop to iterate over list of available nodes
        cost_list = []
        # Obtain cost/distance to go from node_state to available nodes
        ...
        if rrt_dubins.check_collision(new_node):
            ...
            if test_best_node:
                # Add new_node with best parent
                new_node = test_best_node

            rrt_dubins.node_list.append(new_node)  # Storing all valid nodes
            # Find parent in list
            element = find_element_in_list(new_node.parent, rrt_dubins.node_list)
            #print('----------------------------')
            #print('Node is:', len(rrt_dubins.node_list)-1)
            #print('Parent node is:', element)
            #print('Nearest Nodes:', near_list)
            # Add children to dictionary
            if not "parent{0}".format(element) in children:
                children["parent{0}".format(element)] = []

            children["parent{0}".format(element)].append([len(rrt_dubins.node_list)-1])
            #print(children["parent{0}".format(element)])
    ...
    if close:
        ...
        # print('Final Cost of path:', end_node.cost)
        ...
        for idx in range(len(near_list)):
            ...
            if rrt_dubins.check_collision(temp_node):
                if temp_node.cost < original_cost:
                    ...
                    #print('New parent chosen')

    return None


def child_cost(rrt_dubins, parent, children):
    # Update cost of children nodes from rewired parent
    if children:
        for idx in range(len(children)):
            # Get new cost
            update_node = rrt_dubins.propogate(parent, rrt_dubins.node_list[children[idx]])
            # Update cost in children node
            print('Child cost updated')
            rrt_dubins.node_list[children[idx]].cost = update_node.cost
    return None
