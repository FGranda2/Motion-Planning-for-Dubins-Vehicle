...
def main():
    ...
    for idx in range(instances+1):
        ...
        cost_rrt.append(path_node_list[-1].cost)
        print('Instance:', idx)
    ...
    for idx in range(instances+1):
        ...
        cost_rrt_star.append(path_node_list[-1].cost)
        print('Instance:', idx)

    np.savetxt('data_RRT2.csv', cost_rrt, delimiter=',')
    np.savetxt('data_RRT_star2.csv', cost_rrt_star, delimiter=',')
    binwidth = 2

    plt.hist(cost_rrt, bins=range(int(np.rint(min(cost_rrt))), int(np.rint(max(cost_rrt)) + binwidth), binwidth), alpha=0.5,label='RRT')
    plt.hist(cost_rrt_star, bins=range(int(np.rint(min(cost_rrt_star))), int(np.rint(max(cost_rrt_star)) + binwidth), binwidth), alpha=0.5,label='RRT*')
